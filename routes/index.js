...
router.post('/sign-up', function(req, res, next) {
  userModel.findOne({ email: req.body.emailFromFront }, function(err, user) {
    if (!user) {
...
router.post('/sign-in', function(req, res, next) {
  userModel.findOne({ email: req.body.emailFromFront.toLowerCase() }, function(
    err,
    user
  ) {
...
router.post('/add-city', function(req, res, next) {
  ...
  request(
    'http://api.openweathermap.org/data/2.5/weather?q=' +
      req.body.addedCityFromFront +
      '&appid=fc07f13e149c30c7f3bc9c87c606a95f&units=metric&lang=fr',
